...
import { Bot, Clock, Sparkles, RefreshCw, AlertCircle } from "lucide-react";
...
export default function CaptionTab({ Card, PlaceholderBox, result }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await captionApi.generateCaptions(jobId);
  ...
  async function handleRetry() {
    ...
    setError(null);
    try {
      const generated = await captionApi.generateCaptions(jobId);
  ...
  if (status === "loading" || !captions) {
    return (
      <div className="space-y-4">
        ...
        <p className="text-[11px] text-slate-600 text-center italic animate-pulse">
          Generating captions via AI…
        </p>
      </div>
    );
  }
