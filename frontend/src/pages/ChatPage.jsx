...
import {
  ...
  Film, Check, Search, ChevronDown, ChevronUp, X, Info, Save,
} from "lucide-react";
...
function Thumbnail({ jobId, alt }) {
  ...
  return (
    <div className="relative w-20 h-[45px] rounded overflow-hidden shrink-0 bg-slate-900 border border-slate-700/50">
      {state === "loading" && <div className="absolute inset-0 bg-slate-800 animate-pulse" />}
      {state === "error" && (
      ...
      <img src={src} alt={alt}
        ...
        className={`w-full h-full object-cover transition-opacity duration-300 ${state === "done" ? "opacity-100" : "opacity-0"}`}
      />
    </div>
  );
}
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      <div className={`shrink-0 w-7 h-7 rounded-full flex items-center justify-center
        ...
        {isUser ? <User size={13} className="text-blue-400" /> : <Bot size={13} className="text-slate-400" />}
      </div>
      <div className="max-w-[78%] space-y-1.5">
        ...
        {videosUsed?.length > 0 && (
          <p className="text-[10px] text-slate-600 pl-1">Context: {videosUsed.join(", ")}</p>
        )}
      </div>
    </div>
  );
...
function TypingIndicator() {
  return (
    <div className="flex gap-3">
      <div className="shrink-0 w-7 h-7 rounded-full flex items-center justify-center bg-slate-700/60 border border-slate-600/40">
        <Bot size={13} className="text-slate-400" />
      </div>
      <div className="px-3.5 py-3 rounded-xl rounded-tl-sm bg-slate-800/70 border border-slate-700/40 flex items-center gap-1.5">
        {[0, 1, 2].map((i) => (
...
function ContextPanel({ jobIds, context, onSaved }) {
  const [open,   setOpen]   = useState(false);
  const [fields, setFields] = useState({ location: "", event: "", date: "", notes: "" });
  ...
  async function handleSave() {
    setSaving(true);
    try {
      await fetch(`${API_BASE}/chat/global/context`, {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ job_ids: jobIds, context: fields }),
      });
      onSaved(fields);
      setSaved(true);
  ...
  return (
    <div className="border border-slate-700/50 rounded-xl overflow-hidden shrink-0">
      <button
        onClick={() => setOpen(o => !o)}
      ...
      >
        <div className="flex items-center gap-2">
          <Info size={12} className={hasContext ? "text-blue-400" : "text-slate-600"} />
          <span className="text-[11px] font-semibold text-slate-400">Session Context</span>
          {hasContext && (
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-blue-500/15 text-blue-400 border border-blue-500/25">active</span>
          )}
        </div>
      ...
      {open && (
        <div className="px-4 py-3 space-y-2.5 bg-slate-900/40">
          <p className="text-[10px] text-slate-600">
            Provide context about this session (location, event, etc.) to improve AI responses across all selected videos.
          </p>
          {[
            { key: "location", placeholder: "e.g. Paris, Place de la République" },
            { key: "event",    placeholder: "e.g. Public demonstration, sports event" },
            { key: "date",     placeholder: "e.g. 2024-07-14 15:30" },
            { key: "notes",    placeholder: "Any additional context…" },
          ].map(({ key, placeholder }) => (
            <div key={key}>
              <label className="text-[10px] text-slate-500 capitalize mb-1 block">{key}</label>
              <input
                value={fields[key]}
                onChange={e => setFields(f => ({ ...f, [key]: e.target.value }))}
                placeholder={placeholder}
                className="w-full bg-slate-800/60 border border-slate-700/50 rounded-lg px-2.5 py-1.5
                  text-[11px] text-slate-300 placeholder-slate-600 outline-none
                  focus:border-blue-500/50 transition-colors"
              />
            </div>
          ))}
          <button onClick={handleSave} disabled={saving || jobIds.length === 0}
    ...
    <div className="bg-slate-800/30 border border-slate-700/40 rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs font-semibold text-white">Videos</p>
        ...
        <div className="flex items-center gap-2">
          {selected.length < videos.length
            ? <button onClick={(e) => { e.stopPropagation(); onSelectAll(); }}
                className="text-[10px] text-slate-500 hover:text-blue-400 transition-colors">Select all</button>
            : <button onClick={(e) => { e.stopPropagation(); onClear(); }}
                className="text-[10px] text-slate-500 hover:text-red-400 transition-colors">Clear</button>
          }
          {collapsed ? <ChevronDown size={12} className="text-slate-500" /> : <ChevronUp size={12} className="text-slate-500" />}
        </div>
      </div>
        ...
        <div className="border-t border-slate-700/40">
          ...
          <div className="max-h-[300px] overflow-y-auto">
            {filtered.length === 0 ? (
              ...
              return (
                <div key={video.job_id} onClick={() => onToggle(video.job_id)}
                  ...
                  <div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors
                    ${isSelected ? "bg-blue-600 border-blue-500" : "border-slate-600 hover:border-slate-500"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      ...
                      {video.total_detections > 0 && <span className="text-[10px] text-slate-600">{video.total_detections} det</span>}
                      {video.total_unique > 0 && <span className="text-[10px] text-slate-600">{video.total_unique} obj</span>}
                      {hasCaptions && <span className="text-[10px] px-1 rounded bg-blue-500/10 text-blue-500/70">captioned</span>}
                    </div>
                  </div>
                </div>
              );
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${API_BASE}/chat/global/session`, {
          method:  "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function send(question) {
    ...
    if (!q || loading) return;
    if (selected.length === 0) { setError("Select at least one video before sending."); return; }
    ...
    try {
      const res = await globalChatApi.send(selected, q, history, userContext);
      if (res.user_context) setUserContext(res.user_context);
      setHistory([...next, { role: "assistant", content: res.answer, videosUsed: res.videos_used }]);
    } catch (err) {
      setError(err.message);
      setHistory(next);
    } finally {
  ...
  return (
    <div className="flex h-[calc(100vh-56px)] overflow-hidden">
      <div className="w-80 shrink-0 border-r border-slate-800 flex flex-col gap-3 p-4 overflow-y-auto">
        <div>
          <p className="text-xs font-bold text-white mb-0.5">Global AI Chat</p>
          <p className="text-[11px] text-slate-500">Select videos and provide context to enrich analysis.</p>
        </div>

        {fetching ? (
          <div className="space-y-2">
            {[1,2,3].map((n) => <div key={n} className="h-16 rounded-lg bg-slate-800/60 animate-pulse" />)}
          </div>
        ) : videos.length === 0 ? (
        ...
        {selected.length > 0 && (
          <>
            <ContextPanel
              jobIds={selected}
              context={userContext}
              onSaved={setUserContext}
            />

            <div className="rounded-lg bg-slate-800/40 border border-slate-700/40 px-3 py-2.5 space-y-1.5">
              <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest">Active context</p>
              {selected.map((id) => {
                const v = videos.find((v) => v.job_id === id);
                return (
                  <div key={id} className="flex items-center justify-between gap-2">
                    <p className="text-[11px] text-slate-400 truncate flex-1">{v?.filename ?? id}</p>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-800 shrink-0">
          ...
          {history.length > 0 && (
            <button onClick={reset} className="flex items-center gap-1 text-[11px] text-slate-600 hover:text-slate-400 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {history.length === 0 && !loading && sessLoaded && (
            <div className="space-y-4">
              <p className="text-[11px] text-slate-600 text-center">
                {selected.length === 0 ? "Select a video on the left to begin." : "Ask anything about the selected video(s)."}
              </p>
              {selected.length > 0 && (
        ...
        <div className="px-5 py-3 border-t border-slate-800 shrink-0">
          <div className="flex items-end gap-2">
            <textarea
              ...
              onKeyDown={handleKey}
              placeholder={selected.length === 0 ? "Select a video first…" : "Ask about detections, objects, performance, captions…"}
              rows={1}
            ...
            <button onClick={() => send()}
              ...
              className="shrink-0 w-8 h-8 rounded-lg flex items-center justify-center
                bg-blue-600 hover:bg-blue-500 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
